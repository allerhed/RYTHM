-- Migration: Change duration from seconds to minutes (Fixed)
-- File: 005_duration_seconds_to_minutes_fixed.sql
-- This migration changes all duration measurements from seconds to minutes
-- Split into multiple transactions to handle PostgreSQL enum limitations
...
-- Step 6: Now create a new enum type without duration_s and migrate to it
BEGIN;
...
BEGIN
    ...
    -- Count how many duration_m values we have
    SELECT COUNT(*) INTO converted_count
    FROM (
        SELECT value_1_type FROM sets WHERE value_1_type::text = 'duration_m'
        UNION ALL
        SELECT value_2_type FROM sets WHERE value_2_type::text = 'duration_m'
        UNION ALL
        SELECT default_value_1_type FROM exercises WHERE default_value_1_type::text = 'duration_m'
        UNION ALL
        SELECT default_value_2_type FROM exercises WHERE default_value_2_type::text = 'duration_m'
        UNION ALL
        SELECT default_value_1_type FROM exercise_templates WHERE default_value_1_type::text = 'duration_m'
        UNION ALL
        SELECT default_value_2_type FROM exercise_templates WHERE default_value_2_type::text = 'duration_m'
    ) AS subquery;
    ...
    RAISE NOTICE 'Found % duration_m references in the database.', converted_count;
END $$;