...
DateDurationFieldStandardization: Changed date and duration fields from side-by-side layout to stacked vertical layout (separate rows) on new and edit workout pages for better mobile display. Each field now uses full width for improved touch targets and readability on iOS devices. Previously used constrained flex layout with max-widths which was still too wide on mobile screens.
